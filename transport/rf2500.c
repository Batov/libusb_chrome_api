		       ...
		       struct usb_device *dev)
{
	...
	printf("USB_FET_INTERFACE_CLASS = %d\n", USB_FET_INTERFACE_CLASS);


		printf("bNumInterfaces = %d\n", c->bNumInterfaces);
		printf(" interface cycle  = %d\n", i);
		struct usb_interface *intf = &c->interface[1];
		struct usb_interface_descriptor *desc = &intf->altsetting[0];
		printf("desc->bInterfaceClass = %d\n", desc->bInterfaceClass);
		if (
		    !open_interface(tr, dev, 1))
			return 0;
...
transport_t rf2500_open(const char *devpath, const char *requested_serial)
{
	...
	if (open_device(tr, dev) < 0) {
		printc_err("rf2500: !!!q failed to open RF2500 device\n");
		free(tr);
		return NULL;
	}
